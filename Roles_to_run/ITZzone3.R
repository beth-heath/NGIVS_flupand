...
for (age_groups in 1:5){
  for (countries in country_codes){
    ...
    write_parquet(overall_dt1, sink = here::here('Run', paste0('Eastern and Southern Asia(0-33)',countries,age_groups,'.parquet')), compression = "zstd")
    ...
    write_parquet(overall_dt2, sink = here::here('Run', paste0('Eastern and Southern Asia(34-66)',countries,age_groups,'.parquet')), compression = "zstd")
    rm(overall_dt2)
    gc()

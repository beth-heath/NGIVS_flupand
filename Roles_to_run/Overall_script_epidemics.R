...
#### reading in parameters from the outisde #####

args <- commandArgs(trailingOnly = TRUE)
continent <- as.numeric(args[1])
coverage_scenario <- as.numeric(args[2])
...
for (age_groups in 1:5){
  for (countries in country_codes){
    ...
    epid_dt <- converting_epidemic_code(itz_input,years_of_analysis,1:20, ageing_date, hemisphere_input)
    ...
    write_parquet(overall_dt1, sink = here::here('Run_script',paste0('ITZzone', continent,'_', coverage_scenario), paste0(condensed_c_name, countries,age_groups,'Epidemic_1.parquet')), compression = "zstd")
    rm(overall_dt1)
    gc()
    ...
    epid_dt <- converting_epidemic_code(itz_input,years_of_analysis,21:40, ageing_date, hemisphere_input)
    infs_rds_list <- mclapply(1:length(vacc_type_list), flu_parallel_ITZ_epi, mc.cores=length(vacc_type_list))
    overall_dt2 <- rbindlist(infs_rds_list) %>% reduce_function() %>% arrow_table()
    rm(infs_rds_list)
    gc()
    write_parquet(overall_dt2, sink = here::here('Run_script',paste0('ITZzone', continent,'_', coverage_scenario), paste0(condensed_c_name, countries,age_groups,'Epidemic_2.parquet')), compression = "zstd")
    rm(overall_dt2)
    gc()
    
    #selecting the third set of simulations
    simulation_nos_input <- 41:60
    epid_dt <- converting_epidemic_code(itz_input,years_of_analysis,41:60, ageing_date, hemisphere_input)
    infs_rds_list <- mclapply(1:length(vacc_type_list), flu_parallel_ITZ_epi, mc.cores=length(vacc_type_list))
    overall_dt3 <- rbindlist(infs_rds_list) %>% reduce_function() %>% arrow_table()
    rm(infs_rds_list)
    gc()
    write_parquet(overall_dt3, sink = here::here('Run_script',paste0('ITZzone', continent,'_', coverage_scenario), paste0(condensed_c_name, countries,age_groups,'Epidemic_3.parquet')), compression = "zstd")
    rm(overall_dt3)
    gc()
    
    #selecting the fourth set of simulations
    simulation_nos_input <- 61:80
    epid_dt <- converting_epidemic_code(itz_input,years_of_analysis,61:80, ageing_date, hemisphere_input)
    infs_rds_list <- mclapply(1:length(vacc_type_list), flu_parallel_ITZ_epi, mc.cores=length(vacc_type_list))
    overall_dt4 <- rbindlist(infs_rds_list) %>% reduce_function() %>% arrow_table()
    rm(infs_rds_list)
    gc()
    write_parquet(overall_dt4, sink = here::here('Run_script',paste0('ITZzone', continent,'_', coverage_scenario), paste0(condensed_c_name, countries,age_groups,'Epidemic_4.parquet')), compression = "zstd")
    rm(overall_dt4)
    gc()
    
    #selecting the final set of simulations
    simulation_nos_input <- 81:100
    epid_dt <- converting_epidemic_code(itz_input,years_of_analysis,81:100, ageing_date, hemisphere_input)
    infs_rds_list <- mclapply(1:length(vacc_type_list), flu_parallel_ITZ_epi, mc.cores=length(vacc_type_list))
    overall_dt5 <- rbindlist(infs_rds_list) %>% reduce_function() %>% arrow_table()
    rm(infs_rds_list)
    gc()
    write_parquet(overall_dt5, sink = here::here('Run_script',paste0('ITZzone', continent,'_', coverage_scenario), paste0(condensed_c_name, countries,age_groups,'Epidemic_5.parquet')), compression = "zstd")
    rm(overall_dt5)
    gc()

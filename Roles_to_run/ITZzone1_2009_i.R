### File for ITZzone1 (Africa) for a 2009-like pandemic with infection-period modifying mechanism of action####
...
for (age_groups in 1:5){
  for (countries in country_codes){
    ...
    write_parquet(overall_dt1, sink = here::here('Reduced_run','ITZzone1', paste0('Africa',countries,age_groups,'2009_1i.parquet')), compression = "zstd")
    ...
    rm(infs_rds_list)
    gc()
    write_parquet(overall_dt2, sink = here::here('Reduced_run','ITZzone1', paste0('Africa',countries,age_groups,'2009_2i.parquet')), compression = "zstd")
    rm(overall_dt2)
    gc()
    ...
    rm(infs_rds_list)
    gc()
    write_parquet(overall_dt3, sink = here::here('Reduced_run','ITZzone1', paste0('Africa',countries,age_groups,'2009_3i.parquet')), compression = "zstd")
    rm(overall_dt3)
    gc()
    ...
    rm(infs_rds_list)
    gc()
    write_parquet(overall_dt4, sink = here::here('Reduced_run','ITZzone1', paste0('Africa',countries,age_groups,'2009_4i.parquet')), compression = "zstd")
    rm(overall_dt4)
    gc()
    ...
    rm(infs_rds_list)
    gc()
    write_parquet(overall_dt5, sink = here::here('Reduced_run','ITZzone1', paste0('Africa',countries,age_groups,'2009_5i.parquet')), compression = "zstd")
    rm(overall_dt5)
    gc()

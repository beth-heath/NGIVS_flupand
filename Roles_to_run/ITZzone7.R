...
for (age_groups in 1:5){
  for (countries in country_codes){
    ...
    saveRDS(overall_dt1, file = here::here(paste0('Southern America(0-33)',countries,age_groups,'.rds')))
    rm(overall_file1 )
    rm(overall_dt1)
    ...
    saveRDS(overall_dt2, file = here::here(paste0('Southern America(34-66)',countries,age_groups,'.rds')))
    rm(overall_file2 )
    rm(overall_dt2)
    ...
    saveRDS(overall_dt3, file = here::here(paste0('Southern America(67-100)',countries,age_groups,'.rds')))
    rm(overall_file3 )
    rm(overall_dt3)

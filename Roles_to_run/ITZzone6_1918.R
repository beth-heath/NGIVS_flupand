...
for (age_groups in 1:5){
  for (countries in country_codes){
    ...
    infs_rds_list <- mclapply(1:length(vacc_type_list), function(i) flu_parallel_ITZ(i)[[3]], mc.cores=length(vacc_type_list))
    
    overall_dt1 <- rbindlist(infs_rds_list) %>% arrow_table()
    ...
    infs_rds_list <- mclapply(1:length(vacc_type_list), function(i) flu_parallel_ITZ(i)[[3]], mc.cores=length(vacc_type_list))
    overall_dt2 <- rbindlist(infs_rds_list) %>% arrow_table()
    ...
    overall_dt3 <- rbindlist(infs_rds_list) %>% arrow_table()
    ...
    #run each of these save the summary file then remove with rm
    overall_dt4 <- rbindlist(infs_rds_list) %>% arrow_table()
    ...
    #run each of these save the summary file then remove with rm
    overall_dt5 <- rbindlist(infs_rds_list) %>% arrow_table()

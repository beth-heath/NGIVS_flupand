...
for (age_groups in 1:5){
  for (countries in country_codes){
    ...
    write_parquet(overall_dt1, sink = here::here('Run', paste0('Oceania-Melanesia-Polynesia',countries,age_groups,'1918_1.parquet')), compression = "zstd")
    rm(overall_dt1)
    gc()
    ...
    write_parquet(overall_dt2, sink = here::here('Run', paste0('Oceania-Melanesia-Polynesia',countries,age_groups,'1918_2.parquet')), compression = "zstd")
    rm(overall_dt2)
    gc()
    ...
    write_parquet(overall_dt3, sink = here::here('Run', paste0('Oceania-Melanesia-Polynesia',countries,age_groups,'1918_3.parquet')), compression = "zstd")
    rm(overall_dt3)
    gc()
    ...
    infs_rds_list <- mclapply(1:length(vacc_type_list), flu_parallel_ITZ, mc.cores=length(vacc_type_list))
    overall_file4 <- list(infs_rds_list[[1]][[3]], infs_rds_list[[2]][[3]], infs_rds_list[[3]][[3]], infs_rds_list[[4]][[3]], infs_rds_list[[5]][[3]], infs_rds_list[[6]][[3]])
    #run each of these save the summary file then remove with rm
    overall_dt4 <- rbindlist(overall_file4)
    
    rm(infs_rds_list)
    rm(overall_file4 )
    gc()
    
    overall_dt4 <- arrow_table(overall_dt4)
    
    write_parquet(overall_dt4, sink = here::here('Run', paste0('Oceania-Melanesia-Polynesia',countries,age_groups,'1918_4.parquet')), compression = "zstd")
    ...
    infs_rds_list <- mclapply(1:length(vacc_type_list), flu_parallel_ITZ, mc.cores=length(vacc_type_list))
    overall_file5 <- list(infs_rds_list[[1]][[3]], infs_rds_list[[2]][[3]], infs_rds_list[[3]][[3]], infs_rds_list[[4]][[3]], infs_rds_list[[5]][[3]], infs_rds_list[[6]][[3]])
    #run each of these save the summary file then remove with rm
    overall_dt5 <- rbindlist(overall_file5)
    
    rm(infs_rds_list)
    rm(overall_file5 )
    gc()
    
    overall_dt5 <- arrow_table(overall_dt5)
    
    write_parquet(overall_dt5, sink = here::here('Run', paste0('Oceania-Melanesia-Polynesia',countries,age_groups,'1918_5.parquet')), compression = "zstd")

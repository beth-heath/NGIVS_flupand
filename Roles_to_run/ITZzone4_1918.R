...
for (age_groups in 1:5){
  for (countries in country_codes){
    ...
    write_parquet(overall_dt1, sink = here::here('Run', paste0('Europe1',countries,age_groups,'1918.parquet')), compression = "zstd")
    rm(overall_dt1)
    gc()
    ...
    write_parquet(overall_dt2, sink = here::here('Run', paste0('Europe2',countries,age_groups,'1918.parquet')), compression = "zstd")
    rm(overall_dt2)
    gc()
    ...
    write_parquet(overall_dt3, sink = here::here('Run', paste0('Europe3',countries,age_groups,'1918.parquet')), compression = "zstd")
    rm(overall_dt3)
    gc()

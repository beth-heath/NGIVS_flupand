...
health_dat_annual %>%
  ...
  ggplot() + 
  geom_point(aes(time_of_pandemic, that_year_pand_base_i/8e9), col = 'red') + 
  geom_point(aes(time_of_pandemic, that_year_pand_ngiv_i/8e9), col = 'blue') + 
...
ggplot() +
  ...
  theme_bw() +
  scale_x_continuous(limits = c(0.5, 24.5), breaks = 1:24, labels = c(rbind(rep('', 6), rep('', 6), unname(who_region_labs), rep('', 6)))) +
...
comp_out_pop_meds <- comp_out_pop %>% 
  ...
  summarise(diff_in_threshold_cost = median(diff_in_threshold_cost),
            regional_mean_tc = median(regional_mean_tc))
...
comp_out_pop_meds %>% 
  ...
  mutate(diff_in_threshold_cost = case_when(diff_in_threshold_cost < 1 ~ 1, T ~ diff_in_threshold_cost),
         regional_mean_tc = case_when(regional_mean_tc < 1 ~ 1,T ~ regional_mean_tc)) %>%
  ggplot() +
  ...
  theme(text = element_text(size = 14),
        ...
        strip.background = element_rect(color=NA, fill=NA, linewidth=0.5),
        axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  # ggtitle('National and regional median additional economic benefit per dose, by year of pandemic occurrence') + 
...
merged_SA_data_plot %>% 
  ...
  geom_point(aes(x = regional_mean_tc, y = ANALYSIS, col = ANALYSIS), size = 3) +
...
supp_plot_dat %>% 
  ...
  geom_point(data = supp_plot_dat %>% filter(indices == 1),
             aes(x = regional_mean_tc, y = ANALYSIS, col = ANALYSIS), size = 3) +
  geom_point(data = supp_plot_dat %>% filter(indices == 1),
             aes(x = regional_mean_tc, y = ANALYSIS), size = 3, alpha = 0.2) +
  facet_grid(WHO_region ~ pandemic_scenario,
             scales = 'free',
             labeller = labeller(WHO_region = who_region_labs,
                                 age_testing_strategy = supp.labs.age,

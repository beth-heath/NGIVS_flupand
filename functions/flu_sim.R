...
one_flu <- function(
  ...
  if(!epid_start_date==period_start_date & max(dt$time)==end_date){
    ...
    output <- rbind(data.table(time = time_before, 
                               ...
                               IU1 = 0, IU2 = 0, IU3 = 0, IU4 = 0,
                               IV1 = 0, IV2 = 0, IV3 = 0, IV4 = 0,
                               IVR1 = 0, IVR2 = 0, IVR3 = 0, IVR4 = 0), dt)
  }
  if(epid_start_date==period_start_date & !max(dt$time)==end_date){
    time_after <- seq.Date(max(dt$time)+7, end_date, by=7)
    output <- rbind(dt,
...
)
{
  ...
  #this is the old code - still need to go through and make sure makes sense but changing so only have one date
  #NOTE - need to check why this is happening though

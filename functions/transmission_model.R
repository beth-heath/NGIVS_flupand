...
incidence_VS <- function(
  ...
  mod <- flu_odin$new(
    ...
    gammav1 = 2/calendar_input$gamma[1] ,
    gammav2 = 2/calendar_input$gamma[2],
    vacc_rel_inf = vacc_rel_inf,
    vacc_stop_inf = vacc_stop_inf,
  ...
  yVR <- mod$transform_variables(y_run)$cumIVR
  ...
  output_y <- cbind(time = y$time, y[,V1:V4], yU[,V1:V4], yV[,V1:V4], yVR[,V1:V4])
  colnames(output_y) <- c('time', 'I1', 'I2', 'I3', 'I4',
                          'IU1', 'IU2', 'IU3', 'IU4', 'IV1', 'IV2', 'IV3', 'IV4', 'IVR1', 'IVR2', 'IVR3', 'IVR4')
...
flu_odin <- odin::odin({
  ...
  # Transmission matrix
  sij[,] <- cij[i,j] * (I1[j] + I2[j] + vacc_rel_inf*(I1v[j] + I2v[j]+ I1vR[j] + I2vR[j]) )
  ...
  deriv(cumI[]) <- newInf[i] + newInfv[i] + newInfvR[i]
  deriv(cumIU[]) <- newInf[i] 
  deriv(cumIV[]) <- newInfv[i]
  deriv(cumIVR[]) <- newInfvR[i]
  ...
  initial(E2vR[1:no_groups]) <- 0
  initial(I1vR[1:no_groups]) <- 0
  initial(I2vR[1:no_groups]) <- 0
  initial(RnvR[1:no_groups]) <- 0
  initial(cumIVR[1:no_groups]) <- 0
...
fcn_vaccinated_demography <- function(
  ...
  initial_infected_vector <- c(initial_infected, rep(0,8))
  #have not changed this as looks like this sets susceptibility to 0 
